...
/**
 * Returns the COUNT of articles which match the query - if present
 */
module.exports.getArticleCounts = function (query) {
    return connect().then((db) => {
        if (query) {
            return db.collection("articles").count({ tags: query })
        }
        else {
            return db.collection("articles").count();
        }
    })
}
...
module.exports.getPeopleCount = function (query) {
    return connect().then((db) => {
        if (query) {
            return db.collection("people").count({ $text: { $search: query, $caseSensitive: false } });
        } else {
            return db.collection("people").count();
        }
    })
}
...
module.exports.getArticlesByPersonCount = function (name) {
    return connect().then((db) => {
        return db.collection("articles").count({ "links.people.name": name });
    });
}
